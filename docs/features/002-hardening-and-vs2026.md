...
| H | CI test-step shape: **keep Quick (unit, 96) + Full (unit + integration, 204) as designed** (2026-08-25) | T6 unchanged. The 96 unit cases run twice per CI run; that duplication is accepted in exchange for exact parity with the local `test:quick` / `test:full` commands, so any CI failure reproduces locally with one named command. T9 measures its cost |
