...
|-----------|-------|-----------------|
| Extension architecture analysis | **Decided — no migration.** The extension stays on the in-process VSSDK model; `Microsoft.VisualStudio.Extensibility` is not adopted. Feature 002 slice S7 records the decision, its evidence, and the conditions that would reopen it. | Four depended-upon capabilities have no verified out-of-process replacement (Open Folder/workspace, debugging launch, the `ITestContainerDiscoverer` → Test Explorer bridge, and MEF). Revisit trigger: Microsoft announcing deprecation or reduced support for the 17.x VSSDK APIs this extension uses. |
...
| Candidate | Scope | Priority signal |
|-----------|-------|-----------------|
| Dependency conflicts and quality gates | Resolve all `MSB3277` conflicts and remove the sole warning exception; redesign and, if still desired, re-enable DRY, mutation, and CRAP against real production targets. Also: **own the Rust nightly pin's staleness.** Feature 002 T1 pinned the toolchain to a dated channel (`.github/rust-nightly-channel`) with no renewal mechanism, no expiry, and no drift signal — deferred as D10. Suggested shape: a `scope=External` test asserting the pin is younger than N days, reusing that trait's existing manual/scheduled, outside-the-deterministic-gate semantics. Availability is not the risk (dated nightlies are archived indefinitely); big-bang drift is. | Deferred from feature 001 as P0. No later product feature should weaken the now-green mandatory gates. |
...
| Cargo process and build coordination | Prevent stale incremental-debug binaries and indefinite Cargo build-directory locks; define ownership, cancellation, bounded waits, working directory, and environment behavior. Also a one-line consistency cleanup surfaced by feature 002 T1: `FileScanner.cs:74` filters on `Constants.ManifestFileExtension` (`".toml"`) while `FileScannerFactory.cs:15` registers the scanner on the name-exact `Constants.ManifestFileName` (`"Cargo.toml"`). The broad check is dead breadth — unreachable, no behaviour change — but it contradicts its caller's contract and should be tightened to `IsManifest`. | Open issues [#44](https://github.com/kitamstudios/rust-analyzer.vs/issues/44) and [#45](https://github.com/kitamstudios/rust-analyzer.vs/issues/45), plus residual reports in closed [#34](https://github.com/kitamstudios/rust-analyzer.vs/issues/34). |
| Editor and LSP configuration parity | Design coherent settings for inlay hints, brace completion, Navigation Bar, rust-analyzer flags/environment, suggestions, Quick Info timing/size/color, and Code Definition Window support. | Repeated Marketplace and GitHub demand: [#22](https://github.com/kitamstudios/rust-analyzer.vs/issues/22), [#28](https://github.com/kitamstudios/rust-analyzer.vs/issues/28), [#35](https://github.com/kitamstudios/rust-analyzer.vs/issues/35), [#46](https://github.com/kitamstudios/rust-analyzer.vs/issues/46), [#47](https://github.com/kitamstudios/rust-analyzer.vs/issues/47), [#48](https://github.com/kitamstudios/rust-analyzer.vs/issues/48), and [#49](https://github.com/kitamstudios/rust-analyzer.vs/issues/49). No longer gated on the extension-model analysis: that analysis established migration would **not** close these gaps (Microsoft issue [#426](https://github.com/microsoft/VSExtensibility/issues/426) confirms `workspace/configuration` was missing from the out-of-process model). Feasibility here is a protocol-level question — probe the existing LSP broker for what rust-analyzer exposes and what the VS client surfaces. |
